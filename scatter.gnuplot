...
plot \
    ...
    plotfile u (c(xcol)==0&&c(ycol)==0?NaN:column(satcol)==0||column(famcol)!=7?NaN:clamp(xcol)):(clamp(ycol)) ls 7 pt 4 ps 0.7 ti 'Rand sat', \
    ...
    plotfile u (c(xcol)==0&&c(ycol)==0?NaN:column(satcol)==1?NaN:clamp(xcol)):(clamp(ycol)) ls 1 pt 6 ps 0.2 ti 'Any unsat', \
    x w l ls 0 notitle
